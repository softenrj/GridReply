...
import { CREATE_POLL, REVEAL_ANSWERS, RESET_POLL } from '../../../utils/api/APIConstants';
import OrganizerOptions from './OrganizerOptions';
...
function Organizer({ pollId, sessionId, onPoll, poll }: { pollId: string | null, sessionId: string, onPoll: (poll: Poll) => void, poll: Poll | null }) {
    ...
    React.useEffect(() => {
        ...
        if (poll.options && poll.options.length > 0) {
            setOptions(poll.options.map((option) => ({ ...option, id: Number(option.id) })));
        }
    ...
    const handleRevealAnswers = async () => {
        try {
            ...
            const res = await postApi<ApiResponse<any>>({
                url: REVEAL_ANSWERS + `/${poll._id}`,
                values: {}
            })

            if (res?.success) {
                setIsRevealed(true);
                toast.success("Answers revealed!");
            } else if (!res?.success && res?.message) {
                toast.error(res.message);
            }
        } catch (error: any) {
    ...
    return (
        <div className="h-fit w-full flex flex-col gap-4 mx-auto my-6 p-6 sm:p-8 bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 font-sans">
            <form
                onSubmit={handleSubmit}
            >
                <div className='flex flex-col items-start'>
                    <div className="flex items-center gap-3 border-b border-gray-100 mb-2 pb-2">
                        <div className="p-2 bg-orange-50 rounded-xl">
                            <Carrot fill="#ED9121" strokeWidth={1.5} className="w-6 h-6 text-orange-500" />
                        </div>
                        <h2 className="uppercase tras tracking-widest text-transparent bg-gradient-to-r from-red-500 to-pink-500 bg-clip-text font-bold text-lg">
                            Organizer
                        </h2>
                    </div>

                    <div className="flex items-center gap-3 mb-6">
                        <h2 className='uppercase text-[#1d1344] tracking-wider font-bold'>Code: </h2>
                        <p className="uppercase tracking-widest text-transparent bg-linear-to-l from-[#60c7ff] to-[#006aff] bg-clip-text font-bold">
                            {sessionId}
                        </p>
                    </div>
                </div>
            ...
            <OrganizerOptions onRevealAnswers={handleRevealAnswers} onResetGrid={handleResetGrid} isRevealed={isRevealed} />
